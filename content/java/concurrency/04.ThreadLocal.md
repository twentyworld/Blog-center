...
![img](04.ThreadLocal.assets/ThreadLocal-2.png)
...
`Thread`类有一个类型为`ThreadLocal.ThreadLocalMap`的实例变量`threadLocals`，也就是说每个线程有一个自己的`ThreadLocalMap`。`ThreadLocalMap`有自己的独立实现，可以简单地将它的`key`视作`ThreadLocal`，`value`为代码中放入的值(实际上`key`并不是`ThreadLocal`本身，而是它的一个弱引用)。每个线程在往某个`ThreadLocal`里塞值的时候，都会往自己的`ThreadLocalMap`里存，读也是以某个`ThreadLocal`作为引用，在自己的`map`里找对应的`key`，从而实现了线程隔离。

[![img](04.ThreadLocal.assets/2745643-20220705221352646-853996788.png)](https://img2022.cnblogs.com/blog/2745643/202207/2745643-20220705221352646-853996788.png)

**一个`ThreadLocal`只能存储一个`Object`对象，如果需要存储多个`Object`对象那么就需要多个`ThreadLocal`。**

[![img](04.ThreadLocal.assets/2745643-20220705221454246-1404614846.png)](https://img2022.cnblogs.com/blog/2745643/202207/2745643-20220705221454246-1404614846.png)
...
![img](04.ThreadLocal.assets/1824337-20200829092808143-824603771.png)
...
![img](04.ThreadLocal.assets/webp-20250621170315229)
...
![img](04.ThreadLocal.assets/webp-20250621170331832)
...
![img](04.ThreadLocal.assets/webp-20250621170449761)
...
![img](04.ThreadLocal.assets/webp-20250621170515101)
...
![img](04.ThreadLocal.assets/webp-20250621170532894)
...
![img](04.ThreadLocal.assets/webp-20250621170541989)

这里，slotToExpunge的指向就是前面的脏entry.

**前向没有脏entry 后向环形查找找到可覆盖的entry**

![img](04.ThreadLocal.assets/webp-20250621170630918)

这里，slotToExpunge的指向的就是向后循环所查找到的第一个脏entry.

**前向没有脏entry 2.2后向环形查找未找到可覆盖的entry**

![img](04.ThreadLocal.assets/webp-20250621170644483)
