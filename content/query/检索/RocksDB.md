...
- [RocksDB](#rocksdb)
  ...
  - [1. Basic Operations](#1-basic-operations)
    - [1.1 LSM 与 WriteBatch](#11-lsm-与-writebatch)
    - [1.2 同步写与异步写](#12-同步写与异步写)
    - [1.3 Snapshots](#13-snapshots)
    - [1.4 Slice & PinnableSlice](#14-slice--pinnableslice)
    - [1.5 Transactions](#15-transactions)
    - [1.6 Column Family](#16-column-family)
    - [1.7 MemTable and Table factories](#17-memtable-and-table-factories)
    - [1.8 Block size](#18-block-size)
    - [1.9 Writer Buffer](#19-writer-buffer)
    - [1.10 Key Layout](#110-key-layout)
    - [1.11 Checksums](#111-checksums)
    - [1.12 Approximate Sizes](#112-approximate-sizes)
    - [1.13 Purging WAL files](#113-purging-wal-files)
    - [1.14 Prefix Iterators](#114-prefix-iterators)
    - [1.15 Multi-Threaded Compactions](#115-multi-threaded-compactions)
    - [1.16 Incremental Backups and Replication](#116-incremental-backups-and-replication)
    - [1.17 Thread Pool](#117-thread-pool)
    - [1.18 Bloom Filter](#118-bloom-filter)
    - [1.19 Time to Live](#119-time-to-live)
  - [2 RocksDB Memory](#2-rocksdb-memory)
    - [2.1 Block Cache](#21-block-cache)
    - [2.2 Indexes and bloom filters](#22-indexes-and-bloom-filters)
    - [2.3 Indexes and bloom filters](#23-indexes-and-bloom-filters)
    - [2.4 Blocks pinned by iterators](#24-blocks-pinned-by-iterators)
    - [2.5 读流程](#25-读流程)
    - [2.6 memory pool](#26-memory-pool)
  - [3 Block Cache](#3-block-cache)
  - [4 Column Families](#4-column-families)
    - [4.1 Option](#41-option)
    - [4.2 Set Up Option](#42-set-up-option)
    - [4.3 WriteOption & Persistence](#43-writeoption--persistence)
  - [5 Flush & Compaction & Merge](#5-flush--compaction--merge)
    - [5.1 Leveled Compaction](#51-leveled-compaction)
      - [5.1.1 Compaction Score](#511-compaction-score)
      - [5.1.2 Level Max Bytes](#512-level-max-bytes)
      - [5.1.3 compact file](#513-compact-file)
      - [5.1.4 compaction](#514-compaction)
      - [5.1.5 并行 Compact 与 sub-compact](#515-并行-compact-与-sub-compact)
    - [5.2 Universal Compaction](#52-universal-compaction)
    - [5.4 Merge](#54-merge)
  - [6 磁盘文件](#6-磁盘文件)
    - [**6.1 log 文件**](#61-log-文件)
    - [6.2 Manifest 文件](#62-manifest-文件)
    - [6.3 SSTfile](#63-sstfile)
    - [6.4 memtable](#64-memtable)
    - [6.5 VersionSet](#65-versionset)
    - [6.6 MetaData Restore](#66-metadata-restore)
    - [6.7 Snapshot](#67-snapshot)
      - [6.7.1 Checkpoints](#671-checkpoints)
    - [6.8 Backup](#68-backup)
      - [6.8.1 Backup 编程接口](#681-backup-编程接口)
      - [6.8.2 Backup 性能优化](#682-backup-性能优化)
      - [6.8.3 高级编程接口](#683-高级编程接口)
  - [7 FAQ](#7-faq)
  - [参考文档](#参考文档)
...
![img](http://alexstocks.github.io/pic/log_structured_merge_tree.png)
...
![img](http://alexstocks.github.io/pic/rocksdb_arch.png)
...
‘Options::minwritebuffernumberto_merge’ 设定了把写 buffer 的数据固化到磁盘上时对多少个 buffer 的数据进行合并然后再固化到磁盘上。这个值如果为 1，则 L0 层文件只有一个，这会导致读放大，这个值太小会导致数据固化到磁盘上之前数据去重效果太差劲。
...
![img](http://alexstocks.github.io/pic/rocksdb_read_process.png)
...
![img](http://alexstocks.github.io/pic/rocksdb_mempool.png)

使用内存池时，RocksDB 的内容分配代码模块如下：

![img](http://alexstocks.github.io/pic/rocksdb_mempool_arena.png)
...
![img](http://alexstocks.github.io/pic/rocksdb_write_process.png)
...
![img](http://alexstocks.github.io/pic/rocksdb_subcompaction.png)
...
![img](http://alexstocks.github.io/pic/leveldb_arch.png)
...
![img](http://alexstocks.github.io/pic/rocksdb_log_block_record.png)

如上图，log 文件的逻辑单位是 Record，物理单位是 block，每个 Record 可以存在于一个 block 中，也可以占用多个 block。Record 的详细结构见上图文字部分，其 type 字段的意义见下图。

![img](http://alexstocks.github.io/pic/rocksdb_log_record.png)
...
![img](http://alexstocks.github.io/pic/leveldb_manifest.jpg)
...
![img](http://alexstocks.github.io/pic/leveldb_sst_file.png)
...
![img](http://alexstocks.github.io/pic/leveldb_sst_block.jpg)

record 结构如下图：

![img](http://alexstocks.github.io/pic/leveldb_sst_record.jpg)

Footer 结构如下图：

![img](http://alexstocks.github.io/pic/leveldb_sst_footer.jpg)
...
![img](http://alexstocks.github.io/pic/leveldb_versionset.png)
...
![img](http://alexstocks.github.io/pic/leveldb_version_manifest.png)

新增数据正好对应于VersionEdit内容，也就是说Manifest文件记录的是一组VersionEdit值，在Manifest中的一次增量内容称作一个Block。

![img](http://alexstocks.github.io/pic/leveldb_manifest_block_item.png)

Manifest Block 的详细结构如上图所示。

![img](http://alexstocks.github.io/pic/leveldb_data_restore.png)
...
![img](http://alexstocks.github.io/pic/rocksdb_backup_dir_list.png)
